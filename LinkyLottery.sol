...
// File: contracts/LinkyLottery.sol
...
contract LinkyLottery is ERC721, VRFConsumerBase {
    ...
    external
  {
    ...
    uint256 currentPrice = s.mul(uint256(linkUsdFeed.latestAnswer()), 1e10);
    require(_amount >= s.mul(s.div(s.mul(TICKET_COST, 1e18), currentPrice), 1e10), "Insufficient LINK to purchase");
    ...
    counter++;
    if (treasury < vrfFee) {
      treasury = s.add(treasury, _amount);
    } else {
      currentPot = s.add(currentPot, _amount);
    }
  }
  ...
  function fulfillRandomness(bytes32, uint256 _winningNumber) internal override {
    ...
    unclaimedPot = s.add(currentPot, unclaimedPot);
    delete currentPot;
  ...
  function claimLottery(uint256 _tokenId, uint256 _winningNumber) external {
    ...
    require(s.sub(block.timestamp, RANDOMNESS_FREQUENCY) < tickets[_tokenId].purchaseTime, "Ticket expired");
  ...
  function acceptingPurchases() public view returns (bool) {
    return s.sub(block.timestamp, PURCHASE_FREQUENCY) > lastCalled;
  }

  function canInitiateRandomnessRequest() public view returns (bool) {
    return s.sub(block.timestamp, RANDOMNESS_FREQUENCY) > lastCalled;
  }
}
